...
function CourseForm({ course, busy, onCancel, onSave }: { course: AdminCourse | null; busy: string; onCancel: () => void; onSave: (course: Partial<AdminCourse> & Pick<AdminCourse, "title">) => Promise<void> }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    void onSave({ id: course?.id, title, description, category, instructorName, accessType, requiredLevel, status, sortOrder: course?.sortOrder ?? 0 });
  };
...
function LessonComposer({ courseId, moduleId, nextOrder, busy, act, lesson }: { courseId: string; moduleId: string; nextOrder: number; busy: string; act: ActionRunner; lesson?: AdminLesson }) {
  ...
  const [content, setContent] = useState(() => lessonBodyText(lesson?.body));
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const body = lessonType === "quiz"
      ? { text: content.trim(), quiz: { name: title.trim(), requiresPassing: true, passingPercent, questions: quizPrompt.trim() && options.length >= 2 ? [{ prompt: quizPrompt.trim(), options, correctOptionIndex: Math.min(options.length - 1, Math.max(0, correctOption - 1)), explanation: options[Math.min(options.length - 1, Math.max(0, correctOption - 1))] }] : [] } }
      : { text: content.trim() };
    await act("lesson", () => saveAdminLesson({ id: lesson?.id, moduleId, title, lessonType, body, videoUrl, transcript: lesson?.transcript ?? "", resources: lesson?.resources ?? [], durationSeconds: Math.max(0, durationMinutes * 60), sortOrder: lesson?.sortOrder ?? nextOrder, status }), lesson ? "Lesson updated." : "Lesson added.");
    if (!lesson) { setTitle(""); setContent(""); setVideoUrl(""); setQuizPrompt(""); setQuizOptions(""); }
  };
  return <details className="admin-subcomposer" open={Boolean(lesson)}><summary><FilePlus2 size={14} /> {lesson ? "Edit lesson" : "Add lesson"}</summary><form onSubmit={(event) => void submit(event)}>
    ...
    {lessonType === "quiz" && <div className="admin-quiz-builder"><label>Question<input value={quizPrompt} onChange={(event) => setQuizPrompt(event.target.value)} /></label><label>Answer choices<textarea rows={4} value={quizOptions} onChange={(event) => setQuizOptions(event.target.value)} placeholder={'One choice per line'} /></label><div className="admin-form-grid"><label>Correct choice number<input type="number" min="1" value={correctOption} onChange={(event) => setCorrectOption(Number(event.target.value))} /></label><label>Passing score<input type="number" min="1" max="100" value={passingPercent} onChange={(event) => setPassingPercent(Number(event.target.value))} /></label></div></div>}
    ...
    <button className="secondary-button" disabled={busy === "lesson" || !title.trim()}><Check size={15} /> Save lesson</button>
  </form></details>;
}
...
function LessonRow({ lesson, courseId, busy, act }: { lesson: AdminLesson; courseId: string; busy: string; act: ActionRunner }) {
  ...
  const upload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    await act(`asset-${lesson.id}`, () => uploadAdminLessonAsset(file, courseId, lesson.id), "Lesson file uploaded.");
    event.target.value = "";
  };
...
function CommunityAdmin({ snapshot, busy, act }: AdminSectionProps) {
  ...
  return <section className="admin-panel">
    ...
    <details className="admin-composer"><summary><Plus size={16} /> Add channel</summary><form onSubmit={(event) => { event.preventDefault(); if (!categoryName.trim()) return; void act("category", () => saveAdminCategory({ name: categoryName, sortOrder: snapshot.categories.length }), "Channel created.").then(() => setCategoryName("")); }}><label>Channel name<input value={categoryName} onChange={(event) => setCategoryName(event.target.value)} maxLength={40} required /></label><button className="secondary-button" disabled={busy === "category"}><Plus size={15} /> Add channel</button></form></details>
    ...
    <div className="admin-record-list">{snapshot.reports.filter((report) => ["open", "reviewing"].includes(report.status)).map((report) => <article className="admin-report" key={report.id}><span><small>{report.contentType} · {formatDate(report.createdAt)}</small><strong>{report.reason}</strong></span><div><button disabled={Boolean(busy)} onClick={() => void act(`report-${report.id}`, () => updateAdminReport(report.id, "reviewing"), "Report marked for review.", false)}>Review</button><button disabled={Boolean(busy)} onClick={() => void act(`report-${report.id}`, () => updateAdminReport(report.id, "resolved"), "Report resolved.", false)}>Resolve</button><button disabled={Boolean(busy)} onClick={() => void act(`report-${report.id}`, () => updateAdminReport(report.id, "dismissed"), "Report dismissed.", false)}>Dismiss</button></div></article>)}{!snapshot.reports.some((report) => ["open", "reviewing"].includes(report.status)) && <EmptyAdmin text="No unresolved reports." />}</div>
...
function EventForm({ event, busy, onCancel, onSave }: { event: AdminEvent | null; busy: string; onCancel: () => void; onSave: (event: Partial<AdminEvent> & Pick<AdminEvent, "title" | "startsAt" | "endsAt">) => Promise<void> }) {
  ...
  return <details className="admin-composer" open={Boolean(event)}><summary><Plus size={16} /> {event ? `Edit ${event.title}` : "Create an event"}</summary><form onSubmit={(formEvent) => { formEvent.preventDefault(); if (!title.trim()) return; void onSave({ id: event?.id, title, description, kind, hostName, startsAt: new Date(startsAt).toISOString(), endsAt: new Date(endsAt).toISOString(), meetingUrl, status }); }}><label>Event title<input value={title} onChange={(change) => setTitle(change.target.value)} required /></label><label>Description<textarea rows={3} value={description} onChange={(change) => setDescription(change.target.value)} /></label><div className="admin-form-grid"><label>Type<select value={kind} onChange={(change) => setKind(change.target.value as AdminEvent["kind"])}><option value="live">Live</option><option value="workshop">Workshop</option><option value="office_hours">Office hours</option></select></label><label>Host<input value={hostName} onChange={(change) => setHostName(change.target.value)} /></label></div><div className="admin-form-grid"><label>Starts<input type="datetime-local" value={startsAt} onChange={(change) => setStartsAt(change.target.value)} required /></label><label>Ends<input type="datetime-local" value={endsAt} onChange={(change) => setEndsAt(change.target.value)} required /></label></div><label>Meeting URL<input type="url" value={meetingUrl} onChange={(change) => setMeetingUrl(change.target.value)} /></label><label>Status<select value={status} onChange={(change) => setStatus(change.target.value as AdminStatus)}><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></label><div className="admin-form-actions"><button className="primary-button" disabled={busy === "event"}>Save event</button>{event && <button type="button" className="secondary-button" onClick={onCancel}>Cancel edit</button>}</div></form></details>;
}
...
function CertificateTemplateForm({ template, busy, onSave }: { template: CertificateTemplate | null; busy: string; onSave: (template: Partial<CertificateTemplate> & Pick<CertificateTemplate, "name">) => Promise<void> }) {
...
type ActionRunner = (label: string, action: () => Promise<unknown>, success: string, contentChanged?: boolean) => Promise<void>;
...
function lessonBodyText(body?: Record<string, unknown>) { if (!body) return ""; return typeof body.text === "string" ? body.text : typeof body.content === "string" ? body.content : ""; }
