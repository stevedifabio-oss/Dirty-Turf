    ...
    <div className="view-content community-view">
      ...
      <div className="community-product-layout">
        ...
        <main className="community-main" id="community-discussion">
          <section className="community-header"><div><p className="kicker">7 Figure Turf Cleaning</p><h2>Answers from people doing the work.</h2></div><button className="member-count" onClick={() => setMembersOpen(true)} aria-label="Open member directory"><span className="avatar-pair">{members.slice(0, 2).map((member) => <i key={member.cloudId ?? member.id}>{member.initials}</i>)}</span><strong>{members.length}</strong><Users size={15} /></button></section>
          ...
          })}{visible.length === 0 && <div className="empty-state"><Search size={24} /><h3>No discussions here yet</h3><p>Choose another feed or start the first conversation.</p></div>}</section>
        </main>
        ...
        <aside className="community-context-rail">
          <section className="community-overview-card" id="community-about">
            ...
            <div className="community-group-copy"><h3>7 Figure Turf Cleaning</h3><p>Connect, learn, and grow with turf cleaning owners and operators building stronger companies.</p><button onClick={() => setMembersOpen(true)}>View members <ChevronRight size={15} /></button></div>
