...
Deno.serve(async (request) => {
  ...
  if (!appUrl || request.headers.get("origin") !== new URL(appUrl).origin) {
    return jsonResponse(request, { error: "Billing management is available on the web app only" }, { status: 403 });
  }
  ...
  if (!supabaseUrl || !serviceRoleKey || !stripeSecret) {
    return jsonResponse(request, { error: "Billing is not configured" }, { status: 503 });
  }
  ...
  if (!customer?.provider_customer_id) {
    return jsonResponse(request, { error: "No billing account is connected" }, { status: 404 });
  }
  ...
  try {
    ...
    return jsonResponse(request, { error: "Billing portal could not be opened" }, { status: 502 });
  }
});
